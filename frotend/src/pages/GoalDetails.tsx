...
const EditableField: React.FC<EditableFieldProps> = ({ value, onChange, isEditing, onSave }) => {
  if (isEditing) {
    return (
        ...
        <EditableField
          ...
          onChange={(value) => {
            if (goal) {
              const updatedGoal = {
                ...goal,
                result: { ...goal.result, objective: value }
              };
              setGoal(updatedGoal);
            }
          }}
          ...
          onSave={() => {
            // Use the current value from the state, not the original value
            if (goal) {
              handleSave('objective', goal.result.objective);
            }
          }}
        />
      </div>
            ...
            {task.duration && (
              <div className="timeline-box">
                <div className="duration-display">
                  ...
                  {/* Removed duration edit button */}
                </div>
                
                {editingFields[`duration_${index}`] && (
                  <div className="task-edit-form">
                    <input
                      value={task.duration}
                      onChange={(e) => {
                        if (goal) {
                          const updatedTasks = [...goal.result.tasks];
                          updatedTasks[index] = { ...task, duration: e.target.value };
                          setGoal({
                            ...goal,
                            result: { ...goal.result, tasks: updatedTasks }
                          });
                    ...
                    <div className="form-actions">
                      <button 
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          handleSave(`duration_${index}`, task.duration, index);
                        }}
                      >
                        Save
                      </button>
                      <button
                        className="edit-task-button"
                        onClick={() => setEditingFields(prev => ({ ...prev, [`duration_${index}`]: false }))}
                      >
                        Cancel
                      </button>
                    </div>
